...
| Archivo | Qué hace |
|---------|----------|
| `neusi` | Menú interactivo: conectar por SSH, ver estado, refrescar puertos, abrir dashboard. |
| `neusi-refresh` | Actualiza en el backend los puertos ngrok actuales (modo auto y manual). |

Ambos se exponen como comandos vía symlinks en `~/.local/bin/`.
